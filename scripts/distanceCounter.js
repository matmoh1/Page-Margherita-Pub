...
if(geo) {
    geo.getCurrentPosition(function(location) {
        ...
        message = `Restaurants are located at a distance of:\nRestaurant in Wola - ${distance.toFixed(2)}km,\nRestaurant in Rembertów - ${distanceR.toFixed(2)}km,\nRestaurant in Wilanów - ${distanceW.toFixed(2)}km.\n\n`;
    ...
    detect.addEventListener('click', function() {
        if (distance <= 5 || distance <= 5 || distance <= 5) {
            alert(`${message}Come as you can see we are just around the corner ;)`)
        }
        else {
            alert(`${message}We are waiting for you`)
        }
    });
